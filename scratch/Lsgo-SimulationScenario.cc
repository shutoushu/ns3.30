...
VanetRoutingExperiment::ProcessOutputs ()
{
  ...
  // double SIGO = shutoushu.RoutingProtocol ()->getDistance (2, 2, 2, 2);
  // std::cout << SIGO;
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
  ...
  else if (m_scenario == 3)
    {
      ...
      m_traceFile = "src/wave/examples/no_signal/no_signal_500.tcl"; //モビリティ入力ファイル2

      m_logFile = "src/wave/examples/LSGO_Grid/LSGO.log"; //出力ファイル

      m_mobility = 1;
      m_nNodes = 500; //変更すべきところ
      m_TotalSimTime = 36; //変更すべきところ
      m_nodeSpeed = 0;
      m_nodePause = 0;
      m_CSVfileName = "Raleigh_Downtown50_vanet-routing-compare.csv";
      m_CSVfileName = "Raleigh_Downtown50_vanet-routing-compare2.csv";
