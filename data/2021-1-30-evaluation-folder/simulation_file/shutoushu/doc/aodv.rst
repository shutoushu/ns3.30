...
Ad Hoc On-Demand Distance Vector (SHUTOUSHU)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (SHUTOUSHU) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 SHUTOUSHU model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the SHUTOUSHU-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the SHUTOUSHU model lives in the directory `src/shutoushu`.

Design
++++++

Class ``ns3::shutoushu::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::shutoushu::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::shutoushu::RouteInput``,
...
this functionality are attributes of ``ns3::shutoushu::RoutingProtocol``. 
...
SHUTOUSHU discovers routes on demand.  Therefore, the SHUTOUSHU model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in shutoushu-rqueue.cc. A smart pointer to 
...
* This SHUTOUSHU implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from SHUTOUSHU-UU implementation and can be disabled.
...
the assertion from the SHUTOUSHU RFC that SHUTOUSHU works over UDP.  This model uses
