...
Ad Hoc On-Demand Distance Vector (LSGO)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (LSGO) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 LSGO model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the LSGO-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the LSGO model lives in the directory `src/lsgo`.

Design
++++++

Class ``ns3::lsgo::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::lsgo::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::lsgo::RouteInput``,
...
this functionality are attributes of ``ns3::lsgo::RoutingProtocol``. 
...
LSGO discovers routes on demand.  Therefore, the LSGO model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in lsgo-rqueue.cc. A smart pointer to 
...
* This LSGO implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from LSGO-UU implementation and can be disabled.
...
the assertion from the LSGO RFC that LSGO works over UDP.  This model uses
