...
The source code for the SAMPLE model lives in the directory `src/sample`.

Design
++++++

Class ``ns3::sample::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::sample::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::sample::RouteInput``,
...
this functionality are attributes of ``ns3::sample::RoutingProtocol``. 
...
A packet queue is implemented in sample-rqueue.cc. A smart pointer to 
