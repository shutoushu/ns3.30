...
Ad Hoc On-Demand Distance Vector (JBR)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (JBR) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 JBR model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the JBR-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the JBR model lives in the directory `src/jbr`.

Design
++++++

Class ``ns3::jbr::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::jbr::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::jbr::RouteInput``,
...
this functionality are attributes of ``ns3::jbr::RoutingProtocol``. 
...
JBR discovers routes on demand.  Therefore, the JBR model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in jbr-rqueue.cc. A smart pointer to 
...
* This JBR implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from JBR-UU implementation and can be disabled.
...
the assertion from the JBR RFC that JBR works over UDP.  This model uses
