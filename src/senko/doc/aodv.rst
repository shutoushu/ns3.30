...
Ad Hoc On-Demand Distance Vector (SENKO)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (SENKO) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 SENKO model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the SENKO-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the SENKO model lives in the directory `src/senko`.

Design
++++++

Class ``ns3::senko::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::senko::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::senko::RouteInput``,
...
this functionality are attributes of ``ns3::senko::RoutingProtocol``. 
...
SENKO discovers routes on demand.  Therefore, the SENKO model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in senko-rqueue.cc. A smart pointer to 
...
* This SENKO implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from SENKO-UU implementation and can be disabled.
...
the assertion from the SENKO RFC that SENKO works over UDP.  This model uses
