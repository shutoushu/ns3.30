...
danger 　警戒値
*/
  DangerHeader (int32_t nodeid = 0, int32_t posx = 0, int32_t posy = 0, int8_t hopcount = 0,
                int32_t recvtime = 0, int8_t danger = 0, int32_t myposx = 0, int32_t myposy = 0,
  ...
  void
  SetHopCount (int8_t count) //hopcountを足していく関数
  {
    m_hopcount = count;
  }

  int8_t
  GetHopCount () const //hopcount の値を返す
  {
    return m_hopcount;
  }

  void
  SetDanger (int8_t dan) //警戒地をセットする
  {
    m_danger = dan;
  }
  ...
  void
  SetMyPastPosX (int32_t p) //座標をセットする
  { //座標をセットする
    m_pastmyposx = p;
  }

  int32_t
  GetMyPastPosX () //座標の値を返す
  {
    return m_pastmyposx;
  }

  void
  SetMyPastPosY (int32_t p) //座標をセットする
  { //座標をセットする
    m_pastmyposy = p;
  }

  int32_t
  GetMyPastPosY () //座標の値を返す
  {
    return m_pastmyposy;
  }
