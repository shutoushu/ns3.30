...
{
  static TypeId tid =
      TypeId ("ns3::senko::RoutingProtocol")
      ...
      case SENKOTYPE_DANGER: {
        if (trans[id] == 1)
          { //通信可能ノードで違反車両ではい場合
            ...
            double past_dist =
                GetDist (mypastxpoint[id], mypastypoint[id], (double) sourcex, (double) sourcey);
            if (dist > past_dist) //後方からのパケットのみ
              {

                ///////////////////////////////////////////////////////////////////////////////////

                if (mxpoint.count (recvid) == 0)
                  { //受け取ったIDを持っているかどうか
                    ...
                    if (hopcount == 1)
                      {
                        ...
                        std::cout << "In recv broadcast (Node " << id << "Time"
                      ...
                      {
                        rebroadid[recvid] =
                            hopcount; //リブロードキャストで受け取ったIDとホップカウントを保存
                        rebroad_danger[recvid] = danger;
                        std::cout << "In recv rebroadcast (Node " << id << "Time"
                                  << Simulator::Now ().GetMicroSeconds ()
                                  << ")------------------------------------------------------\n";
                  ...
                  { //持っている場合
                    ...
                    if (mdanger[recvid] < danger)
                      { //保持している警戒値と受け取ったWidの警戒値が異なっていたら
                        ...
                        if (hopcount == 1)
                          {
                            ...
                            std::cout
                                << "update In recv broadcast (Node " << id << "Time"
                                << Simulator::Now ().GetMicroSeconds ()
                                << ")------------------------------------------------------\n";
                            //std::cout<<"broadcast reveive hellonode id"<< recvid<<"\n\n";
                            //std::cout<<"broadcast reveive point"<< posx<<"\n\n";
                          ...
                          {
                            rebroadid[recvid] =
                                hopcount; //リブロードで受け取ったIDとホップカウントを保存
                            rebroad_danger[recvid] = danger;
                            //std::cout<<"In recv rebroadcast (Node "<< id<<")------------------------------------------------------\n";
                            std::cout
                                << " update In recv rebroadcast (Node " << id << "Time"
                                << Simulator::Now ().GetMicroSeconds ()
                                << ")------------------------------------------------------\n";
...
void
RoutingProtocol::ReSendXBroadcast (int32_t id, int32_t posx, int32_t posy, int8_t hopcount,
                                   int32_t time, int8_t danger, int sourcex, int sourcey)
{
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin ();
       j != m_socketAddresses.end (); ++j)
    {
      ...
      DangerHeader dangerHeader (id, posx, posy, hopcount, time, danger, sourcex, sourcey);
...
*/
bool
has_key_using_count (std::map<int, int> &mxpoint, int n)
{
...
RoutingProtocol::SetMyPos () //自分の位置を取得してマップに保存
{
  ...
  if (trans[id] != 2)
    { //目的地に到着してなかったら
      ...
      if (myxpoint.count (id) == 0)
        { //すでに自分の前の位置がセットされていないとき
          myxpoint[id] = myposx;
          myypoint[id] = myposy;
        }
      else
        { //前の座標のデータがあるとき
          ...
          mydist = std::sqrt ((myxpoint[id] - myposx) * (myxpoint[id] - myposx) +
                              (myypoint[id] - myposy) *
                                  (myypoint[id] - myposy)); //ｘ秒前の位置と現在の自分の位置の距離
          ////////////////////////////////////x=1のときmydistは秒速に値する
          ...
          //}
          //時速に変換
          if (myposcount[id] > NotransTime && trans[id] == 1)
            { //通信可能ノードが一定期間停止
              trans[id] = 2;
...
{
  double dist = std::sqrt ((x2 - x) * (x2 - x) + (y2 - y) * (y2 - y));
  return dist;
}
      ...
      if (id != i)
        {
          ...
          if (dist < ImageRange)
            {
              if (trans[id] == 1 &&
                  trans[i] == 1) //画像処理ノードとそれに写ってるノードが通信可能のとき
                {
                  areaangle[i] = angle;
                  areadist[i] = dist;
                }
          ...
          if (sendid[itr->first] == 0) //おなじIDの車両のブロードキャストは二度行わない
            {
              ...
              if (mxpoint.count (itr->first) == 0 && trans[itr->first] == 1)
                { //すでにもっていたIDじゃなかったら
                  SendXBroadcast (itr->first, GodXposition[itr->first], GodYposition[itr->first], 1,
                                  time, 0, (int) x, (int) y);
          ...
          if (itr3 != mxpoint.end ())
            { //通信で受け取った違反車両IDが画像処理内にあったら
              ...
              if (dist < Dmin) //速度判定には距離が足りない場合
                ...
                { //速度判定するのに十分な距離があるとき
                  int32_t time =
                      Simulator::Now ().GetMicroSeconds (); //速度判定対象車両を見つけた時間
                  ...
                  if (sendid[itr->first] == 0)
                    {
                      if (speed > Vmin && trans[itr->first] == 1) //違反車両だったら
                        {
                          std::cout << "速度違反車両がいました　IDは" << itr->first << "speed"
                                    << speed << "detect veihcle id" << id << "\n";
                          mdanger[itr->first]++;
                          SendXBroadcast (itr->first, GodXposition[itr->first],
                                          GodYposition[itr->first], 1, time, mdanger[itr->first],
                                          (int) x, (int) y);
                          sendid[itr->first] = 1; //すでに送ったIDのマップを１に変換
                          ...
                          if (detectdanger[itr->first] == 0)
                            {
                              ...
                              detectdanger[itr->first] <
                              mdanger[itr->first]) //警戒地よりdetectdangerが小さければ値を更新する
                            {
                              detectdanger[itr->first] = mdanger[itr->first];
                            }
                        }
...
void
RoutingProtocol::Direction (double current_x, double current_y, double source_x, double source_y)
{
  //int32_t id = m_ipv4->GetObject<Node> ()->GetId ();
