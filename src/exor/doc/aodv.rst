...
Ad Hoc On-Demand Distance Vector (EXOR)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (EXOR) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 EXOR model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the EXOR-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the EXOR model lives in the directory `src/exor`.

Design
++++++

Class ``ns3::exor::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::exor::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::exor::RouteInput``,
...
this functionality are attributes of ``ns3::exor::RoutingProtocol``. 
...
EXOR discovers routes on demand.  Therefore, the EXOR model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in exor-rqueue.cc. A smart pointer to 
...
* This EXOR implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from EXOR-UU implementation and can be disabled.
...
the assertion from the EXOR RFC that EXOR works over UDP.  This model uses
