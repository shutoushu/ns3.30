      ...
      if (id == source_list[time - Grobal_StartTime])
        {
          ...
          // SendGlsgoBroadcast (0, des_list[index_time], m_my_posx[des_list[index_time]], m_my_posy[des_list[index_time]], 1);
  ...
  switch (tHeader.Get ())
    {
      ...
      case GLSGOTYPE_SEND: {
        ...
        ////*********recv hello packet log*****************////////////////
        // std::cout << " id " << id << "が受信したglsgo packetは\n";
        // for (int i = 0; i < 5; i++)
        //   {
        //     if (pri_id[i] != 1000000)
  ...
  if (Simulator::Now ().GetSeconds () == Grobal_StartTime + Grobal_SourceNodeNum + 1)
    {
      ...
      for (auto itr = broadcount.begin (); itr != broadcount.end (); itr++)
        {
          std::cout << "des id " << itr->first << "glsgo broadcast数" << broadcount[itr->first]
                    << "\n";
