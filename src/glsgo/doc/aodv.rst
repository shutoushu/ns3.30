...
Ad Hoc On-Demand Distance Vector (GLSGO)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (GLSGO) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 GLSGO model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the GLSGO-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the GLSGO model lives in the directory `src/glsgo`.

Design
++++++

Class ``ns3::glsgo::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::glsgo::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::glsgo::RouteInput``,
...
this functionality are attributes of ``ns3::glsgo::RoutingProtocol``. 
...
GLSGO discovers routes on demand.  Therefore, the GLSGO model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in glsgo-rqueue.cc. A smart pointer to 
...
* This GLSGO implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from GLSGO-UU implementation and can be disabled.
...
the assertion from the GLSGO RFC that GLSGO works over UDP.  This model uses
