...
Ad Hoc On-Demand Distance Vector (SIGO)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (SIGO) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 SIGO model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the SIGO-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the SIGO model lives in the directory `src/sigo`.

Design
++++++

Class ``ns3::sigo::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::sigo::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::sigo::RouteInput``,
...
this functionality are attributes of ``ns3::sigo::RoutingProtocol``. 
...
SIGO discovers routes on demand.  Therefore, the SIGO model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in sigo-rqueue.cc. A smart pointer to 
...
* This SIGO implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from SIGO-UU implementation and can be disabled.
...
the assertion from the SIGO RFC that SIGO works over UDP.  This model uses
