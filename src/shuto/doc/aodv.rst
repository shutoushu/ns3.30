...
Ad Hoc On-Demand Distance Vector (SHUTO)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (SHUTO) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 SHUTO model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the SHUTO-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the SHUTO model lives in the directory `src/shuto`.

Design
++++++

Class ``ns3::shuto::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::shuto::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::shuto::RouteInput``,
...
this functionality are attributes of ``ns3::shuto::RoutingProtocol``. 
...
SHUTO discovers routes on demand.  Therefore, the SHUTO model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in shuto-rqueue.cc. A smart pointer to 
...
* This SHUTO implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from SHUTO-UU implementation and can be disabled.
...
the assertion from the SHUTO RFC that SHUTO works over UDP.  This model uses
