...
Ad Hoc On-Demand Distance Vector (GSIGO)
---------------------------------------

This model implements the base specification of the Ad Hoc On-Demand 
Distance Vector (GSIGO) protocol. The implementation is based on 
:rfc:`3561`.

The model was written by Elena Buchatskaia and Pavel Boyko of ITTP RAS,
and is based on the ns-2 GSIGO model developed by the CMU/MONARCH group
and optimized and tuned by Samir Das and Mahesh Marina, University of
Cincinnati, and also on the GSIGO-UU implementation by Erik Nordström of 
Uppsala University.

Model Description
*****************

The source code for the GSIGO model lives in the directory `src/gsigo`.

Design
++++++

Class ``ns3::gsigo::RoutingProtocol`` implements all functionality of 
...
forwarding.  The first one, ``ns3::gsigo::RouteOutput``, is used for 
locally originated packets, and the second one, ``ns3::gsigo::RouteInput``,
...
this functionality are attributes of ``ns3::gsigo::RoutingProtocol``. 
...
GSIGO discovers routes on demand.  Therefore, the GSIGO model buffers all 
packets while a route request packet (RREQ) is disseminated. 
A packet queue is implemented in gsigo-rqueue.cc. A smart pointer to 
...
* This GSIGO implementation can detect the presence of unidirectional 
  ...
  This heuristic is taken from GSIGO-UU implementation and can be disabled.
...
the assertion from the GSIGO RFC that GSIGO works over UDP.  This model uses
